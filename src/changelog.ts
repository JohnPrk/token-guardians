...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    body: `
- 새 설정이나 기존 설정값이 없는 경우, 캐릭터를 우클릭하면 바로 새로고침하지 않고 **동작 메뉴**를 먼저 보여줘요. 이미 "즉시 새로고침"을 직접 선택해둔 경우에는 기존 동작이 그대로 유지됩니다.
- 설정의 익명 사용 통계 항목을 다른 설정 카드와 같은 스위치 형태로 정리했어요.
- 소개 페이지에서 사용 중인 OS에 맞는 다운로드 버튼이 강조되고, 캐릭터가 상단 메뉴 위로 겹치지 않게 다듬었습니다.
`,
  },
  {
    version: "2.32.0",
...
`,
  },
  {
    version: "2.31.0",
    date: "2026-06-16",
    title: "크기 조절 핸들 아이콘 개선",
    body: `
- 크기 조절 핸들(오른쪽 아래 모서리) 아이콘을 양방향 화살표로 바꿔, 크기를 조절하는 곳이라는 게 더 분명해졌어요.
`,
  },
  {
    version: "2.30.0",
    ...
    title: "더블클릭으로 계정 전환",
    body: `
- 지키미를 **더블클릭**하면 다음 계정으로 바로 전환돼요. 계정을 두 개 이상 등록했다면 처음 한 번 안내 말풍선도 보여줍니다.
`,
  },
  {
    version: "2.29.0",
    date: "2026-06-16",
    title: "Codex(OpenAI) 계정 지원",
    body: `
- **Codex(OpenAI)** 사용량도 함께 볼 수 있어요. 쿠키 입력 없이 컴퓨터에 저장된 Codex 기록에서 자동으로 읽어옵니다.
- 무료 플랜은 월간 한도를, 유료(Plus/Pro) 플랜은 5시간·주간 한도를 보여줘요.
`,
  },
  {
    version: "2.25.0",
